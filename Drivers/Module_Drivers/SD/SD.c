...
//				printf("card not enter TRANSFER\n");
//				return;
//			}
//			HAL_Delay(10);
//		}
...
//		disk_reinitialize(SDFatFs->drv);
		...
		if ((*fr = f_mount(SDFatFs, (TCHAR const * ) SDPath, 0)) != FR_OK)
		{
			MX_FATFS_DeInit();
			*fr = FR_DISK_ERR;
		}else printf("mount was successful\r\n");
	}else
	{
		*fr = FR_NOT_READY;
	}
...
uint16_t getSDFileNumber(void) {
    ...
    fr = f_findfirst(&dj, &fno, "", "*.csv"); /* Start to search for SSP files */
...
void setFileName(uint16_t *fileCounter,char *SDFileNamePtr)
{
    ...
    *fileCounter = getSDFileNumber() + 1;
    ...
    sprintf(SDFileNamePtr, "FIL%05i.csv", *fileCounter);
