...
//			if ((HAL_GetTick() - tickstart) > 50) {  // 50ms 超时，防止死锁
//				// 可打印错误或 break
//				printf("tx timeout\r\n");
//				break;
//			}
...
//				// 方法2：备选简单延时（如果轮询不支持，根据波特率算：19200bps一帧约几ms）
//				// uint32_t delay_us = modbus_rtu_get_rts_delay(ctx);  // 如果你设置了rts_delay
//				// if (delay_us == 0) delay_us = 500;  // 默认500us，根据实际调
//				// HAL_Delay(delay_us / 1000);  // 或用DWT精确微秒延时
